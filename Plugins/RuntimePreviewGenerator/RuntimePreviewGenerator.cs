...
public static class RuntimePreviewGenerator
{
    ...
    private static Color m_backgroundColor = new Color(0f, 0f, 0f, 0f);
    ...
    public static Texture2D GenerateModelPreview(Transform model, int width = 64, int height = 64, bool shouldCloneModel = false, bool shouldIgnoreParticleSystems = true, bool UseSceneCamera = false, Vector3? Scenecamera = null)
    {
    ...
    {
        GenerateModelPreviewInternal(model, shader, replacementTag, width, height, shouldCloneModel, shouldIgnoreParticleSystems, callback, false);
    }
#endif
...
#endif
    {
        ...
        try
        {
            ...
            if (UseSceneCamera)
            {
                if (sceneCamerapos != null)
                {
                    var localvel = (Vector3)sceneCamerapos;
                    Vector3 localpos = new Vector3(PREVIEW_POSITION.x + localvel.x, PREVIEW_POSITION.y + localvel.y, PREVIEW_POSITION.z + localvel.z);

                    renderCamera.transform.position = localpos;
            ...
            try
            {
                //Debug.Log("Starting Render engine");
                ...
                if (asyncCallback != null)
                {
                    AsyncGPUReadback.Request(renderTexture, 0, m_backgroundColor.a < 1f ? TextureFormat.RGBA32 : TextureFormat.RGB24, (asyncResult) =>
                    {
                        try
                        {
                            //Debug.Log("Started Texture Rendering");
                            ...
                            if (!asyncResult.hasError)
                            {
                                ...
                                //Debug.Log("Loaded Raw Texture Data");
                            }
                            else
                            {
                            ...
                            //Debug.Log("Applied Texture Data");
...
#endif
                {
                    result = new Texture2D(width, height, m_backgroundColor.a < 1f ? TextureFormat.RGBA32 : TextureFormat.RGB24, false);
                    result.ReadPixels(new Rect(0f, 0f, width, height), 0, 0, false);
                    //Debug.Log("Reading Camera Pixels");
                    result.Apply(false, m_markTextureNonReadable);
                    //Debug.Log("Aplying Pixels to Texture");
                }
            }
            finally
            {
